...
data_path = [
    ("./test/TestwithIndeces/" + x + "_0528.geojson") for _, x in label_dict.items()
]
...
np.save("./test/my_patches.npy", X_test)
X_test = X_test.reshape(len(X_test), -1)
np.save("./test/my_labels.npy", y_test)
print(X_test.shape)
print(y_test.shape)
...
pca_bands = []
for i in range(bands_num):
    sub_band = X_test[:, i * 25 : (i + 1) * 25]
    # print(sub_band.shape)
    band_pca = pca.fit_transform(sub_band)
    pca_bands.append(band_pca)
pca_bands = np.stack(pca_bands, axis=1)
pca_X_test = pca_bands.reshape((len(pca_bands), -1))
X_test = pca_X_test[:, support]
print(X_test.shape)
...
# new_X + y; X_test + y_test
model = RF(n_estimators=50)
model.fit(new_X, y)
y_pred = model.predict(X_test)

cm = confusion_matrix(y_test, y_pred)
print(f"accuracy of RF50 : {np.sum(y_pred == y_test) / len(y_pred)}")
